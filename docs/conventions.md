...
由 `frame-me-parent` 提供，业务工程引入 `frame-me-boot` 默认获得统一响应/异常、Redis、审计日志、配置加密、SSE、消息通知、认证抽象与云基础底座（配置刷新/优雅下线）能力；数据访问、多数据源、两级缓存、接口文档、老规范适配、WebSocket、JWT/Sa-Token/RBAC、Nacos 按需引入。能力清单与默认开关见 [architecture.md#脚手架横切能力](./architecture.md)，配置细节见 [frame-me-parent/docs/modules.md](../frame-me-parent/docs/modules.md)。
